...
int main(int argc, char** argv)
{
    ...
    d_a = clCreateBuffer(context, 
        CL_MEM_READ_ONLY, sizeof(float) * count, NULL, &err);
    ...
    d_d = clCreateBuffer(context, 
        CL_MEM_WRITE_ONLY, sizeof(float) * count, NULL, &err);
    ...
    err = clEnqueueWriteBuffer(commands, 
        d_a, CL_TRUE, 0, sizeof(float) * count, h_a, 
        0, NULL, NULL);
    checkError(err, "Copying h_a to device at d_a");

    err = clEnqueueWriteBuffer(commands, 
        d_b, CL_TRUE, 0, sizeof(float) * count, h_b, 
        0, NULL, NULL);
    checkError(err, "Copying h_b to device at d_b");

    err = clEnqueueWriteBuffer(commands, 
        d_c, CL_TRUE, 0, sizeof(float) * count, h_c, 
        0, NULL, NULL);
    checkError(err, "Copying h_b to device at d_d");
    ...
    err = clEnqueueReadBuffer(commands, 
        d_d, CL_TRUE, 0, sizeof(float) * count, h_d, 
        0, NULL, NULL );  
    if (err != CL_SUCCESS)
    {
    ...
    for(i = 0; i < count; i++)
    {
        ...
        if(tmp*tmp < TOL*TOL)        // correct if square deviation is less than tolerance squared
            ...
            printf(
                " tmp %f h_d %f h_a %f h_b %f h_c %f \n",
                tmp, h_d[i], h_a[i], h_b[i], h_c[i]);
        }
    }

    // summarise results
    printf(
        "D = A+B+C:  %d out of %d results were correct.\n", 
        correct, count);
