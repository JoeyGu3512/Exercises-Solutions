   ...
   const int N
    ...
    for(int ni=0;ni<num_i;ni++){
        ...
        if((ni*lsz_i+lid_i)<N && gid_j<N){    // prevent outbound
            B_col_cache[lid_i] = B[(ni*lsz_i+lid_i)*N+gid_j];
        }
        ...
        // compute stride
        for(int k=0;k<lsz_i;k++){
            if((ni*lsz_i+k)<N && gid_i<N){
                dot_prod += A[gid_i*N+(ni*lsz_i+k)] * B_col_cache[k];
            }
        }

        // barrier usage of local mem
        barrier(CLK_LOCAL_MEM_FENCE);
